...
class SchemeAdmin(admin.ModelAdmin):
    """Класс для настройки отображения модели схема"""

    list_display = ("id", "product", "supplier_link", "debt", "created_at")
    list_filter = ("node__city",)
    actions = ["clear_debt"]

    def supplier_link(self, obj):
        url = f"/admin/suppliers/supplier/{obj.node.id}/change/"
        return format_html('<a href="{}">{}</a>', url, obj.node.name)
